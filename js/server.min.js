...
class user {
  ...
  card() {
    ...
    $('#o_usdt').html(1*(1*res.usdt.o).toFixed(4)); $('#f_usdt').html(1*(1*res.usdt.f).toFixed(4));
    ...
    Object.keys(_metamask.nets).forEach(id => {
      ...
      $('#o_'+id).html(1*(1*res[id].o).toFixed(4)); $('#f_'+id).html(1*(1*res[id].f).toFixed(4));
    })
  ...
  async withdraw(e) {
    if(typeof e == 'object') {
      ...
      if(isNaN(x) || x == 0) {
        ...
        var t1 = _user.usdt >= 100 || this.nets[this.id].name.indexOf('Testnet') > 0;
        var t2 = 1*_user.card()[this.id].f > 1*x+fee;
    ...
    if(typeof e == 'number') {
      ...
      if(n.name.indexOf('Testnet') > 0) $('#vip3').hide()
      else $('#vip3').show()
