...
$(document).ready(async() => {
  ...
  for (const [c, n] of Object.entries(_mm.coins)) {
    ...
    var mark = c == 'USDT' ? '' : `<a id="m_${c}" onclick="market('${c}')" style="cursor: pointer"> ${c}/USDT</a>`
    $('#metamask').append(`<tr>
    <td><img src="img/${coin}.png" width="32" height="32" style="vertical-align: -10px;">&nbsp;&nbsp;${coin}</td>
    <td>${n.name} ${c == 'ETH_t' ? '(<a href="https://goerlifaucet.com" target="_blank">FREE</a>)' : c == 'XEENUS_t' ? '(<a id="xeenus" class="disabled">FREE</a>)' : c == 'BNB_t' ? '(<a href="https://testnet.bnbchain.org/faucet-smart" target="_blank">FREE</a>)' :  ''}</td>
    <td>${mark}</td>
    <td id="o_${c}">0</td><td id="f_${c}">0</td>
  ...
  if(_mm.metamask) {
    ...
    $('#deposit_s').click(() => {
      _mm.deposit(db.coin,1*$('#deposit_v').val()+1*$('#deposit_f').html());
      $('#deposit_v').val(0); $('#deposit_f').html(0.0001);
      $(`#deposit_s`).removeClass("waves-effect waves-light submit").addClass('disabled');
    });

    $('#withdraw_v').change(e => withdraw(e));
    $('#withdraw_s').click(() => {
      _mm.withdraw(db.coin, 1*$('#withdraw_v').val(),1*$('#withdraw_f').html());
      $('#withdraw_v').val(0); $('#withdraw_f').html(0.0001);
      $(`#withdraw_s`).removeClass("waves-effect waves-light submit").addClass('disabled');
    });

    $('.market_v').change(e => market(e));
    $('#market_sell').click(() => {
      _user.to_server({ cmd: 'market', coin: db.coin, side: 'sell', price: 1*$('#market_sp').val(), qty: 1*$('#market_sq').val() });
      $('#market_sp').val(0); $('#market_sq').val(0);
      $(`#market_sell`).removeClass("waves-effect waves-light submit").addClass('disabled');
    });
    $('#market_buy').click(() => {
      _user.to_server({ cmd: 'market', coin: db.coin, side: 'buy', price: 1*$('#market_bp').val(), qty: 1*$('#market_bq').val() });
      $('#market_bp').val(0); $('#market_bq').val(0); $('#market_max').val(0);
      $(`#market_buy`).removeClass("waves-effect waves-light submit").addClass('disabled');
    });
  }
...
function deposit(e) {
  ...
  if(_mm.coins[e]) {
    $('#deposit_h').html(_mm.coins[e].name);
    $('#card').hide(); $('#vip').hide(); $('.deposit').show();
    history(e);
  }
}
...
function withdraw(e) {
  if(typeof e == 'object') {
    ...
    if(isNaN(x) || x <= 0) {
      ...
      var c = _user.card();
      var t1 = c.USDT.o + c.USDT.f >= 10 || _mm.coins[db.coin].name.indexOf('Testnet') > 0;
      var t2 = 1*c[db.coin].f > 1*x+fee;
  ...
  if(_mm.coins[e]) {
    ...
    $('#withdraw_h').html(n.name);
    $('#card').hide(); $('#vip').hide(); $('.withdraw').show();
    if(n.name.indexOf('Testnet') > 0) $('#vip_10').hide()
    else $('#vip_10').show();
    history(e);
  }
}
...
function market(e) {
  if(typeof e == 'object') {
    var id = e.target.id;
    if(id == 'market_sp' || id == 'market_sq') {
      var p = $('#market_sp').val(), q = $('#market_sq').val();
      var t1 = isNaN(p) || p <= 0, t2 = isNaN(q) || q <= 0;
      var x = _user.card()[db.coin].f; $('#market_ab').html(x.toFixed(4));
      if(t1 || t2 || x < q) $(`#market_sell`).removeClass("waves-effect waves-light submit").addClass('disabled');
      else $(`#market_sell`).removeClass("disabled").addClass('waves-effect waves-light submit');
    }
    if(id == 'market_bp' || id == 'market_bq') {
      var p = $('#market_bp').val(), q = $('#market_bq').val();
      var t1 = isNaN(p) || p <= 0, t2 = isNaN(q) || q <= 0;
      var x = _user.card()['USDT'].f; $('#market_max').html((x/p).toFixed(4));
      if(t1 || t2 || p*q > x) $(`#market_buy`).removeClass("waves-effect waves-light submit").addClass('disabled');
      else $(`#market_buy`).removeClass("disabled").addClass('waves-effect waves-light submit');
    }
    return
  }

  if(_mm.coins[e]) {
    $('#card').hide(); $('#vip').hide(); $('.market').show();
    history(e);
    _user.to_server({ cmd: 'subscribe', coin: e });
  }
}
...
function history(e) {
  ...
  $('#market_ab').html(_user.card()[e].f);

  $('#deposit_tdody').html(''); $('#withdraw_tdody').html('');
  _user.metamask.forEach(x => { if(x.coin == e) {
    if(x.qty >= 0) {
      $('#deposit_tdody').append(`<tr>
        <td>${new Date(x.date).toLocaleString()}</td>
        <td>${x.hash == 'Invitation fee' ? x.hash : '<a href="'+n.explorer+'/tx/'+x.hash+'" target="_blank">'+x.hash+'</a>'}</td>
        <td>${1*x.qty}</td>
  ...
  $('#market_tdody').html('');
  _user.market.forEach(x => { if(x.coin == e && x.qty != x.exec) {
    $('#market_tdody').append(`<tr>
    <td>${new Date(x.date).toLocaleString()}</td>
    <td>${x.side}</td>
    <td>${x.price}</td>
    <td>${x.qty}</td>
    <td>${x.exec}</td>
    <td><a class="waves-effect waves-light btn" onclick="_user.to_server({ cmd: 'cancel', coin: '${db.coin}', date: ${x.date} });"><i class="material-icons left">delete_forever</i>Cancel</a></td>
  </tr>`)
  }})
}
...
function check_num(e) {
  ...
  return (code == 46 || (code > 47 && code < 58))
}
...
class user {
  ...
  constructor (server) {
    ...
    this.worker.connect(db.server[Number(server.startsWith('https:'))], msg => {

      if(msg.login === true) {
        $('.login').hide(); $("#card").show(); $("#vip").show();
        $('.c_user').html($('#username').val());
        if(msg.demo) $('.c_user:first').append(' (demo)')
      }
      if(msg.login === false) this.captcha('Password is wrong');
      
      if(msg.subscribe) {
        var fb = msg.subscribe.length;
        msg.subscribe.some((e,i) => { if(e[0] == 'buy') { fb = i; return true } });
        var i = 0; for(var j = fb-1; j > fb-11; j-- ) {
          $('#sp_'+i).html(j >= 0 ? msg.subscribe[j][1] : '-');
          $('#sq_'+i).html(j >= 0 ? msg.subscribe[j][2] : '-');
          i++; if(i == 10) break;
        }
        var i = 0; for(var j = fb; j < fb+10; j++) {
          $('#bp_'+i).html(j < msg.subscribe.length ? msg.subscribe[j][1] : '-');
          $('#bq_'+i).html(j < msg.subscribe.length ? msg.subscribe[j][2] : '-');
          i++; if(i == 10) break;
        }
      }

      var f = false;
      if(msg.usdt != undefined) { this.usdt = msg.usdt; f = true }
      if(msg.metamask) {
        msg.metamask.forEach(e => {
          var ff = this.metamask.some((x,i) => {
            if(e.date == x.date) {
              this.metamask[i] = e; return true
            }
          })
      ...
      if(msg.market) {
        msg.market.forEach(e => {
          var ff = this.market.some((x,i) => {
            if(e.date == x.date) {
              if(e.qty + e.exec > 0) this.market[i] = e;
              else this.market.splice(i, 1);
              return true
            }
          })
  ...
  card() {
    ...
    if(res.USDT.f >= 10) $('#vip10').html('active').addClass('green-text')
    else $('#vip10').html('inactive').addClass('red-text')
    
    if(res.USDT.f >= 20) $('#vip20').html('active').addClass('green-text')
    else $('#vip20').html('inactive').addClass('red-text')

    if(res.USDT.f >= 30) $('#vip30').html('active').addClass('green-text')
    else $('#vip30').html('inactive').addClass('red-text')
  ...
  back() {
    $('.xcard').hide(); $('#card').show(); $('#vip').show();
    this.to_server({ cmd: 'subscribe', coin: 'x' });
  }
  to_server(obj) {
