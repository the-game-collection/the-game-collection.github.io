var db = {}, platform = '', tab = '', _user, server = window.location.href

var coins = {
  usdt: { name: 'Tether (USDT) [ERC-20]', cf: 2 },
  eth: { name: 'Ethereum (ETH)', cf: 5 }
}
...
$(document).ready(async() => {
  ...
  $('a').click(function(e) {
    ...
    var w = $(`#${e.target.id}_i`).val(); if(w) navigator.clipboard.writeText(w);
    ...
    if($(e.target).html() == 'person') {
      if($('#person').is(':visible')) { $('#person').hide(); _user.worker.start() }
      else { $('#person').show(); _user.worker.stop() }
    }
  });
...
async function load(P,T) {
  ...
  var list = await $.getJSON(`${platform}/${tab == '#' ? '%23' : tab}.json`);
  $('#list').html('');
  list.forEach(g => {
...
class user {
  ...
  constructor (server) {
    ...
    this.worker.connect(db.server[Number(server.startsWith('https:'))], data => {
      ...
      if(data.w) this.b = data.b;
      if(data.w) this.s = data.s;
      if(data.w) this.m = data.m;
      
      if(data.login === true) {
        $('.login').hide(); $("div[id='card']").show();
        $("span[id='c_user']").html($('#username').val()); $("h5[id='c_user']").html('https://the-game-collection.github.io?id='+$('#username').val());
    ...
    $('.m_connect').click(() => this.connect('metamask'));
    $('.deposit').click(e => this.deposit(e));
    $('.withdraw').click(e => this.withdraw(e));
    $('.back').click(() => this.back());
    $('.c_value').change(e => this.c_value(e));
    $('.submit').click(e => this.submit(e));    
  }
  captcha(msg) {
    this.a = []
  ...
  deposit(e) {
    $('#deposit_c').html(coins[e.target.id].name);
    $('#card').hide(); $('#deposit').show();
  }
  withdraw(e) {
    $('#withdraw_c').html(coins[e.target.id].name);
    $('#card').hide(); $('#withdraw').show();
  }
  c_value(e) {
    var x = $(e.target).val(), dw = e.target.id.split('_')[0];
    if(isNaN(x) || x == 0 || !this.wallet_address) {
      $(`#${dw}_done`).removeClass("waves-effect waves-light submit").addClass('disabled');
      $(`#${dw}_a`).html('-'); $(`#${dw}_f`).html('-');
    }
    else {
      $(`#${dw}_done`).removeClass("disabled").addClass('waves-effect waves-light submit');
      var cf = 2;
      for (const [key, value] of Object.entries(coins)) {
        if($(`#${dw}_c`).html() == value.name) { cf = value.cf; break }
      }
      $(`#${dw}_a`).html(x); $(`#${dw}_f`).html((x/100).toFixed(cf))
    }
  }
  submit(e) {
  ...
  back() {
    $('#deposit').hide(); $('#withdraw').hide(); $('#card').show();
  }
  random(min, max) { 
