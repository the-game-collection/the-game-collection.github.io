...
class user {
  ...
  constructor (server) {
    ...
    this.worker.connect(db.server[Number(server.startsWith('https:'))], data => {
      ...
      if(data.metamask) { this.metamask = this.metamask.concat(data.metamask); f = true }
      if(data.market) { this.market = this.market.concat(data.market); f = true }
...
class metamask {
  ...
  constructor() {
    ...
    for (const [id, n] of Object.entries(this.nets)) {
      $('#metamask').append(`
      <tr>
      <td><img src="img/${n.coin}.png" width="32" height="32"></td>
      <td>${n.name} (${n.coin}) ${n.name.indexOf('Goerli Testnet') > -1 ? '[1]' : n.name.indexOf('Chain ') > -1 ? '[2]': ''}</td><td id="o_${id}">0.0000</td><td id="f_${id}">0.0000</td>
  ...
  async withdraw(e) {
    if(typeof e == 'object') {
      ...
      if(isNaN(x) || x == 0) {
        ...
        var fee = 1*(x/100).toFixed(4); if(fee == 0) fee = 0.0001;
        $(`#withdraw_f`).html(fee);
        if(1*_user.card()[this.id].f > 1*x+fee) $(`#withdraw_s`).removeClass("disabled").addClass('waves-effect waves-light submit');
