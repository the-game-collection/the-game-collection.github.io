...
function game(id) {
  ...
  var fn = g.dir+'\\'+decodeURIComponent(g.url).split('/').pop().slice(0,-4);
...
async function load(P,T) {
  ...
  var list = await $.getJSON(`${server}/${platform}/${tab == '#' ? '0' : tab}.json`);
  ...
  list.forEach(g => {
    if(g.url) {
	    ...
	    $('#list').append(str);
      g.url = myurl(g.url);
	    g.dir = __dirname+'\\downloads\\'+platform;
      g.file = g.dir+'\\'+decodeURIComponent(g.url).split('/').pop();
	    db.list.push(g);
	    game(platform+'_'+i);
	    i++
	  }
...
function region(str) {
  var f = str.toLowerCase().split(',').some(e => {
    if(['usa','europe','australia','asia','japan','korea','mexico','germany','france','russia'].indexOf(e) > -1) return true
  })
  if(f) return str
}
...
function langs(str) {
  var f = str.toLowerCase().split(',').some(e => {
    if(['en','de','fr','es','it','jp','ru'].indexOf(e) > -1) return true
  })
  if(f) return str
}
...
function onOpen(e) {
  ...
  var reg = '', lang = '', arr = g.file.split('\\').pop().split('(');
  if(arr.length > 1) {
    arr.shift();
    arr.forEach(e => {
      var str = e.split(')')[0].replace(' ','');
      if(!reg) reg = region(str);
      if(!lang) lang = langs(str)
    })
  }
    ...
    <table style="table-layout: fixed;">
      ...
      <tr>
        ...
        <td style="vertical-align: top;">
          ...
          ${reg ? '<p><strong>Region:</strong> '+reg+'</p>' : ''}
          ${lang ? '<p><strong>Languages:</strong> '+lang+'</p>' : ''}
...
function filesize(g) {
  return new Promise((resolve) => {
    if(g.filesize) resolve(g.filesize)
    else {
      var xhr = new XMLHttpRequest();
      xhr.open('GET', g.url, true);
      xhr.setRequestHeader('Range', 'bytes=0-0');
      xhr.onerror = () => { resolve(0) }
      xhr.onload = () => {
        if(xhr.status == 200 || xhr.status == 206) resolve(1*xhr.getResponseHeader('Content-Range').split('/')[1])
        else resolve(0)
      };
      xhr.send(null);
    }
...
class emu {
  ...
  load() {
    ...
    this.dm = new dmaster('emu', { url: url, file: file, dir: __dirname })
  }
  start(params) {
...
class dmaster {
  ...
  constructor(id,g) {
    ...
    filesize(g).then(res => {
      ...
      this.size = res;
      if(fs.existsSync(this.file) && fs.statSync(this.file).size == this.size) {
        this.proc = [1,1]; this.show(); return
      }
  ...
  show() {
    ...
    if(this.proc[0] == this.proc[1]) {
      if((this.file.slice(this.file.length - 4)).toLowerCase() == '.zip') this.unzip()
      else this.start(false);
      return 100
    }
    return proc > 99 ? 99 : proc
  }
  async start(x) {
    if(x == false) this.stop = true
    else this.stop = !this.stop;
    if(this.stop) {
  ...
  async unzip() {
    ...
    for await (const e of zip) {
      if(e.path != 'cover.gif' && e.path != 'readme.txt') {
    ...
    $("p[id='"+this.id+"']").html('');
    $("div[id='"+this.id+"']").css('width','0%');
    if(this.dir == __dirname) fs.unlinkSync(this.file);
